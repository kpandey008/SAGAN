...
class Generator(nn.Module):
    def __init__(self, z_dim=100, conv_dim=64, n_heads=1):
        ...
        self.layer1 = nn.Sequential(
            spectral_norm(nn.ConvTranspose2d(self.z_dim, 8 * self.conv_dim, 4, 2, 1)),
            nn.BatchNorm2d(8 * self.conv_dim),
            nn.ReLU(),
        )

        self.layer2 = nn.Sequential(
            spectral_norm(nn.ConvTranspose2d(8 * self.conv_dim, 4 * self.conv_dim, 4, 2, 1)),
            nn.BatchNorm2d(4 * self.conv_dim),
            nn.ReLU(),
        )

        self.layer3 = nn.Sequential(
            spectral_norm(nn.ConvTranspose2d(4 * self.conv_dim, 2 * self.conv_dim, 4, 2, 1)),
            nn.BatchNorm2d(2 * self.conv_dim),
            nn.ReLU(),
        )

        self.layer4 = nn.Sequential(
            spectral_norm(nn.ConvTranspose2d(2 * self.conv_dim, self.conv_dim, 4, 2, 1)),
            nn.BatchNorm2d(self.conv_dim),
            nn.ReLU(),
        )

        self.output = nn.Sequential(
            nn.ConvTranspose2d(self.conv_dim, self.conv_dim, 4, 2, 1),
            nn.ReLU(),
            nn.ConvTranspose2d(self.conv_dim, 3, 4, 2, 1),
            nn.ReLU()
        )
