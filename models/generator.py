...
class SAGANGenerator(nn.Module):
    def __init__(self, z_dim, in_channels, n_heads=1):
        ...
        self.block4 = GeneratorSNBlock(64, 64)  # 64 x 64
...
class Generator(nn.Module):
    def __init__(self, z_dim=128, conv_dim=64, n_heads=1):
        ...
        self.layer2 = nn.Sequential(
            spectral_norm(
                nn.ConvTranspose2d(8 * self.conv_dim, 4 * self.conv_dim, 4, 2, 1)
            ),
        ...
        self.layer3 = nn.Sequential(
            spectral_norm(
                nn.ConvTranspose2d(4 * self.conv_dim, 2 * self.conv_dim, 4, 2, 1)
            ),
        ...
        self.layer4 = nn.Sequential(
            spectral_norm(
                nn.ConvTranspose2d(2 * self.conv_dim, self.conv_dim, 4, 2, 1)
            ),
